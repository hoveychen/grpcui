...
WORKDIR /tmp/hoveychen/grpcui
# copy just the files/sources we need to build grpcui
COPY VERSION *.go go.* /tmp/hoveychen/grpcui/
COPY cmd /tmp/hoveychen/grpcui/cmd
COPY internal /tmp/hoveychen/grpcui/internal
COPY standalone /tmp/hoveychen/grpcui/standalone
